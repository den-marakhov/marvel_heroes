...
class ImageUploadService(ImageUploadServiceProtocol):
    ...
    async def upload_image(self, file: UploadFile, hero_id: UUID) -> str:
        if file is None:
            raise ImageProcessingError("File was not provided")
        ...
        try:
            ...
            logger.debug(
                "Image has been validated", hero_id=str(hero_id), filename=file.filename
            )
            ...
            await asyncio.to_thread(
                self._process_image, str(temp_path), str(final_path)
            )

            img_url = f"/{self.upload_dir_path}/{file_id}.webp"
            logger.info("Image has been uploaded successfully", url=img_url)
            ...
            raise ImageProcessingError("Unexpected error while processing image") from e
