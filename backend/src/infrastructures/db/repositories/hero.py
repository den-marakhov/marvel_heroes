...
class HeroRepositorySQLAlchemy(HeroRepositoryProtocol):
    ...
    async def get_heroes(self) -> list[HeroEntity]:
        ...
        try:
            ...
            raise RepositoryReadError("Failed to retrieve heroes from database") from e

    async def get_hero_by_id(self, hero_id: UUID) -> HeroEntity | None:
    ...
    async def update_hero(self, hero_id: UUID, hero_entity: HeroEntity) -> None:
    ...
    async def save_hero(self, hero_entity: HeroEntity) -> None:

        try:
            stmt = select(HeroModel).where(HeroModel.name == hero_entity.name.value)
            result = await self.session.execute(stmt)
            hero_model = result.scalar_one_or_none()
    ...
    async def delete_hero(self, hero_id: UUID) -> None:
