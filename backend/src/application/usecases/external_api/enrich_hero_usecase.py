...
class EnrichHeroUseCase:
    ...
    async def __call__(self, hero_id: UUID, external_id: int) -> HeroDTO:

        external_hero_dto = await self.marvel_hero_api_client.fetch_hero_by_external_id(
            external_id=external_id
        )
        hero_update_dto = self.mapper.from_external_dto_to_updated_dto(
            external_hero_dto
        )
        ...
        logger.info(
            "Hero enriched successfully", hero_id=str(hero_id), external_id=external_id
        )
