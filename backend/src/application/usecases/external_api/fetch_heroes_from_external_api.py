...
class FetchHeroesFromExternalAPIUseCase:
    ...
    async def __call__(self, hero_name: str) -> list[ExternalAPIHeroDTO]:
        logger.info("Starting to fetch heroes from external API", hero_name=hero_name)

        try:
            heroes = await self.marvel_hero_api_client.fetch_hero_by_name(hero_name)
            logger.info(
                "Heroes have been fetched from external API", hero_name=hero_name
            )
            return heroes
